...
// Pasta padrão de segurança (fallback)
...
// Rota para servir arquivos locais dinamicamente
...
// --- CONFIGURAÇÃO DE UPLOAD INTELIGENTE ---
...
// *** NOVA ROTA: LISTAR TODOS OS DOCUMENTOS ***
...
app.post('/documents', dynamicUpload, async (req, res) => {
    try {
        ...
        if (config?.storageType === 'DRIVE' && config.googleDriveJson) {
            ...
            finalUrl = `${req.protocol}://${req.get('host')}/files/${file.filename}`;
            finalType = 'LOCAL';
            finalPath = file.path;
        }
...
app.post('/documents/:id/send-email', async (req, res) => {
    try {
        const d = await prisma.document.findUnique({ where: { id: parseInt(req.params.id) }, include: { client: true } });
        if (d.tipo === 'LOCAL' && d.path) await sendEmail(d.client.email, `Doc: ${d.nome}`, 'Segue anexo', d.path);
        else await sendEmail(d.client.email, `Doc`, `Link: ${d.url}`);
        res.json({ ok: true });
    } catch (e) { res.status(500).json({ erro: e.message }); }
});

// Rotas Padrão (Clientes, Leads, etc)
...
// Leads
app.post('/leads', async (req, res) => { try { const d = req.body; const l = await prisma.lead.create({ data: { ...d, status: "NOVO" } }); res.json(l); } catch (e) { res.status(500).json({ erro: e.message }); } });
...
// Outras
app.get('/appointments', async (req, res) => { const { start, end } = req.query; let w = {}; if (start && end) w.date = { gte: new Date(start), lte: new Date(end) }; const a = await prisma.appointment.findMany({ where: w, include: { client: true }, orderBy: { date: 'asc' } }); res.json(a); });
app.post('/appointments', async (req, res) => { try { const d = req.body; await prisma.appointment.create({ data: { ...d, date: new Date(d.date), clientId: d.clientId ? parseInt(d.clientId) : null } }); res.json({ ok: true }); } catch (e) { res.status(500).json({ erro: e.message }); } });
...
app.post('/financial', async (req, res) => { try { const d = req.body; await prisma.financialRecord.create({ data: { ...d, amount: parseFloat(d.amount), dueDate: new Date(d.dueDate) } }); res.json({ ok: true }); } catch (e) { res.status(500).json({ erro: e.message }); } });
...
app.post('/claims', async (req, res) => { try { const d = req.body; await prisma.claim.create({ data: { ...d, data_ocorrencia: new Date(d.data_ocorrencia), valor_franquia: parseFloat(d.valor_franquia || 0), valor_orcamento: parseFloat(d.valor_orcamento || 0), clientId: parseInt(d.clientId) } }); res.json({ ok: true }); } catch (e) { res.status(500).json({ erro: e.message }); } });
